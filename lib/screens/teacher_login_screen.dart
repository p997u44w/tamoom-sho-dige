...
class _TeacherLoginScreenState extends State<TeacherLoginScreen> {
  ...
  Future<void> _loadRememberedLogin() async {
    final saved = await ApiService.getRememberedLogin('teacher');
  ...
  Future<void> _submit() async {
    ...
    final res = await ApiService.post('auth/login-teacher', {
    ...
    if (res['success'] == true) {
      ...
      if (_rememberMe) {
        await ApiService.saveRememberedLogin('teacher', {
          'school_code': _schoolCodeCtrl.text.trim(),
          'full_name': _nameCtrl.text.trim(),
          'national_code': _nationalCodeCtrl.text.trim(),
        });
      } else {
        await ApiService.clearRememberedLogin('teacher');
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('ورود معلم'), backgroundColor: Colors.transparent, elevation: 0, scrolledUnderElevation: 0),
